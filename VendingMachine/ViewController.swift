...
class ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    @IBAction func purchase() {
        ...
        if let currentSelection = currentSelection {
            ...
            } catch VendingMachineError.insufisontFunds(let amount){
                showAlert("insuficint funds", message: "required Additinal $\(amount) to complete the transaction")
            ...
            // FIXME: Alert the user no selection
        }
    }
    ...
    func showAlert(title: String, message: String? = nil, style: UIAlertControllerStyle = .Alert){
        ...
        let okAction = UIAlertAction(title: "Ok", style: .Default, handler: dismisAlert)
        alertController.addAction(okAction)
        
        presentViewController(alertController, animated: true, completion: nil)
    }
    
    func dismisAlert(sender: UIAlertAction) {
        reset()
        
    }
    
    @IBAction func depostFunds() {
        vendingMachine.depost(5.0)
        updateBalanceLabel()
    }
}
